...

{
  ...
  class Cart {
    ...
    initActions() {
      ...
      thisCart.dom.productList.addEventListener('remove', function(event) { thisCart.remove(event.detail.thisCart);
      });
    }
    add(menuProduct) {
      const thisCart = this;
    ...
    remove(element) {
      const thisCart = this;
      const removeIndex = thisCart.products.indexOf(element);
